...
void change_projet() {
	...
	cadprojetos = fopen("projetos.txt","r+w");
	/*printf("Dados %i\n", dados);
	dados = dados - 1;
	printf("Dados -1 %i\n", dados);*/
	...
	//while(fread(&projetos, sizeof(projetos),1,cadprojetos) == 1) {
		...
		fseek(cadprojetos, (dados-1)*sizeof(projetos),SEEK_SET);
		
		//if(strcmp(dados,projetos.titulo) == 0) {
			...
			printf("Valor do projeto:...... R$ %.2f\n\n", projetos.valor);
			
			printf(":::::: DADOS NOVOS ::::::\n");
			
			projetos.numero = dados;
